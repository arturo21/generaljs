...
import { isFunction, isString, isElement, isObject } from '@pluginjs/is';
import { addClass, removeClass } from '@pluginjs/classes';
import { bindEvent, bindEventOnce } from '@pluginjs/events';
import { parseHTML, query, append, data, remove } from '@pluginjs/dom';
import { translateable, themeable, styleable, eventable, stateable, optionable, register } from '@pluginjs/decorator';
...
var Modal = (_dec = translateable(translations), _dec2 = themeable(), _dec3 = styleable(classes), _dec4 = eventable(events), _dec5 = stateable(), _dec6 = optionable(defaults, false), _dec7 = register(namespace), _dec(_class = _dec2(_class = _dec3(_class = _dec4(_class = _dec5(_class = _dec6(_class = _dec7(_class = class Modal extends GlobalComponent {
  constructor() {
    ...
    this.$container = query(".".concat(this.classes.CONTAINER), this.$element);
    this.setupStates();
    this.setupI18n();
    this.initialize();
    this.bind();
  ...
  open() {
    ...
    addClass("".concat(this.classes.OPEN), query('body'));
    append(this.$element, query(this.options.appendTo));

    if (this.options.overlay) {
      addClass("".concat(this.classes.NAMESPACE, "-fadeIn"), this.$overlay);
      removeClass("".concat(this.classes.NAMESPACE, "-fadeOut"), this.$overlay);
    }

    removeClass("".concat(this.classes.NAMESPACE, "-").concat(this.options.effect, "Out"), this.$container);
    addClass("".concat(this.classes.NAMESPACE, "-").concat(this.options.effect, "In"), this.$container); // trigger open
  ...
  bind() {
    ...
    if (this.options.buttons) {
      bindEvent(this.eventName('click'), event => {
        if (!event.target.classList.contains(this.classes.BUTTON) || !data('action', event.target)) {
          return false;
        }

        var action = data('action', event.target);
  ...
  close() {
    ...
    removeClass("".concat(this.classes.OPEN), query('body')); // close animation

    if (this.options.overlay) {
      addClass("".concat(this.classes.NAMESPACE, "-fadeOut"), this.$overlay);
      removeClass("".concat(this.classes.NAMESPACE, "-fadeIn"), this.$overlay);
    }

    addClass("".concat(this.classes.NAMESPACE, "-").concat(this.options.effect, "Out"), this.$container);
    removeClass("".concat(this.classes.NAMESPACE, "-").concat(this.options.effect, "In"), this.$container);
    ...
    bindEventOnce(this.eventName('animationend'), animationendCallback, this.$element); // trigger close
  ...
  initialize() {
    // Validate data
    this.validate();
    this.$content = query(".".concat(this.classes.CONTENT), this.$element);
    this.$title = query(".".concat(this.classes.TITLE), this.$element);
    this.$closeBtn = query(".".concat(this.classes.CLOSE), this.$element);
    this.$buttons = query(".".concat(this.classes.BUTTONS), this.$element);

    if (this.options.overlay) {
      this.$overlay = query(".".concat(this.classes.OVERLAY), this.$element);
    } // theme
  ...
  static alert() {
    ...
    if (length && isObject(length - 1 < 0 || arguments.length <= length - 1 ? undefined : arguments[length - 1])) {
      opt = deepMerge(opt, length - 1 < 0 || arguments.length <= length - 1 ? undefined : arguments[length - 1]);
      length -= 1;
    }
